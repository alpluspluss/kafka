...
namespace kfk
{
    using SlubInternalAllocator = Allocator<AllocPolicy::SWITCHABLE, 64 * 1024>;
    SlubInternalAllocator slub_alloc;
    ...
    static void *alloc_from_buffer(size_t size)
    {
        /* align to 8 bytes */
        size = (size + 7) & ~7;
        void *ptr = slub_alloc.allocate(size);
        if (!ptr)
            return nullptr;
